...
erDiagram
    ...
    brand ||--o{ product : "has"
    member ||--o{ product_like : "likes product"
    product ||--o{ product_like : "liked by"
    member ||--o{ brand_like : "likes brand"
    brand ||--o{ brand_like : "liked by"
...
### product_like
- `UK: uk_product_like_member_product (member_id, product_id)` — 중복 좋아요 방지 (leftmost prefix로 member_id 조회 겸용)

### brand_like
- `UK: uk_brand_like_member_brand (member_id, brand_id)` — 중복 좋아요 방지 (leftmost prefix로 member_id 조회 겸용)
...
- **예외: product_like, brand_like** — 토글 취소 시 hard delete (물리 삭제). UK 충돌 방지를 위해 `deleted_at` 컬럼 없음
